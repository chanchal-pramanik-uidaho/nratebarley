...
create_plot <- function(data, variety_name, plot_letter) {
  ...
  # Fit linear model
  lm_result <- lm(testwt ~ nsupply, data = plot_data)
  ...
  p <- ggplot(plot_data, aes(nsupply, testwt, color = variety, shape = variety, fill = variety)) +
...
extract_parameters <- function(data, variety_name) {
  lm_result <- lm(testwt ~ nsupply, data = data)
