...
maltbarley_filtered <- maltbarley %>%
  ...
  mutate(proteingkg_0.9 = proteingkg/0.9)
...
create_plot <- function(data, variety_name, plot_letter) {
  ...
  # Fit linear model
  lm_result <- lm(proteingkg_0.9 ~ nsupply, data = plot_data)
  ...
  p <- ggplot(plot_data, aes(nsupply, proteingkg_0.9, color = variety, shape = variety, fill = variety)) +
    ...
    scale_y_continuous(limits = c(0.000001, 170), expand = c(0, 0)) +
    geom_abline(intercept = intercept, slope = slope, size = 1.5, colour = 'black') +
    labs(title = variety_name,
         x = bquote('N-Supply' ~'('(kg ~N ~ha^-1) ),
         y = bquote("Protein (g/kg)")) +
    
    # Label Letter
    annotate("text", x = 10, y = 140, label = plot_letter, hjust = 0, family = "serif", size = 6, face = "bold") +
    
    # R2 Line
    annotate("text", x = 250 + 2, y = 55,  
    ...
    theme_bw() +
    coord_cartesian(xlim = c(0, 460), ylim = c(0.000001, 170))
...
extract_parameters <- function(data, variety_name) {
  lm_result <- lm(proteingkg_0.9 ~ nsupply, data = data)
